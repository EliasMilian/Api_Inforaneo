...
controller.addComment = async (req, res) => {
  try {
    ...
    const establecimiento = await Establecimiento.findOne({ title });

    if (!establecimiento) {
      return res.status(404).json({ error: "Establecimiento no encontrado" });
    }

    establecimiento.comments.push({ rating, opinion });
    await establecimiento.save();

    return res.status(200).json({ message: "Comentario añadido con éxito", data: establecimiento });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error: error.message });
  }
...
// Controlador para obtener los comentarios de un establecimiento usando el título
controller.getComments = async (req, res) => {
  try {
    ...
    return res.status(200).json(establecimiento.comments);
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error: error.message });
  }
